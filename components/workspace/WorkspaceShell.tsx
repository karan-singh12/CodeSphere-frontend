...
export function WorkspaceShell({
  ...
  const handleGenerate = useCallback(
    async (prompt: string, imageUrl?: string) => {
      ...
      try {
        ...
        const res = await apiPost("/api/workspaces/generate-code", {
          workspaceId: currentWorkspaceId,
          userId,
          messages: conversationHistory,
          fileData: fileDataRef.current,
          template: fileDataRef.current?.template || initialTemplate,
        }, {
  ...
  const handleImprove = useCallback(
    async (userRequest: string) => {
      ...
      try {
        const res = await apiPost("/api/workspaces/improve-code", {
          userId,
        ...
        if (res.status === 403) {
          toast.error(
            "Upgrade to Starter or Pro to use Improve with CodeSphere Agent."
          );
